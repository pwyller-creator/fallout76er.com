...
- `/maps/` — treasure-map guide pages + hub. **Static / hand-managed.** No generator exists.
- `sitemap.xml` — static, hand-managed.
- `sitemap-creatures.xml` — generated by `build_creatures.js`.

## Build scripts
- `build_creatures.js` is **not kept on disk** — it's regenerated when needed. It outputs the `/creatures/` pages and `sitemap-creatures.xml`. When creature data changes, regenerate those pages; never hand-edit them.
- **No `build_guides.js` exists.** The `/maps/` pages and `sitemap.xml` are edited by hand.
...
- A clickable entry needs `documented: true` plus `video`, `title`, `desc`, and `meta`. Without those it renders as an inert stub.
