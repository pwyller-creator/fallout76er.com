...
- `resources.js` — external, `defer`-loaded. `SCRAP_DATA`, `INGREDIENT_DATA`, `MASK_DATA` (+ `MASK_QUEST_NEED`, `MASK_TIERS`), `MINERVA_LISTS` (18 regular sale lists; Big Sales are derived as the union of the 3 lists before them — regenerate with the untracked `.claude/minerva-data.js` after a game update), `pbNav()`, and the render/init functions for every Pip-Boy sub-page (Radio, Minerva schedule + current/next stock, Plans Exchange, Scrap/Ingredient Spawn Intel, Slasher Mask checklist, treasure-map widget). Loaded via `<script>` with `?v=`.
